...
INFO.prototype.check_message = function(text, can_lock, nick){
	...
	if(get_factoid !== null)
	{
		this.get_factoid(get_factoid[3], get_factoid[2]);
		return;
	}
...
INFO.prototype.get_factoid = function(about, adj, callback){
	...
	action.get_db_data('/factoid/' + about, function(data){
   		if(data !== null){
   			...
   			var send_factoid = about + ' ' + (data.plural ? 'are' : 'is') + ' ' + _this.article_adj(adjective, _this.vars(data.info));

   			action.say(t.success(send_factoid), 1);
   		} else {
...
INFO.prototype.article_adj = function(adj, info){
	if(adj !== ''){
		var article = info.match(/^(a|an|your|you|he|his|she|her|them|their|the|that|those|it|its)\s(.*)/i);
		if(article !== null){
...
INFO.prototype.vars = function(str){
	...
	for(var word in words)
	{
		...
		str = str.replace(var_reg, function(x){
			var new_word = words[word][action.random_number_between(0,words[word].length - 1)];
			if(x[x.length - 1] === 's' && new_word[new_word.length - 1] !== 's') return new_word + 's';
			return new_word;
		});
	}
