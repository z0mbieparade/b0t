module.exports = class INFO{
	constructor(){
		...
		this.db = new DB({
			...
			on_load: function(rt)
			{
				b.log.debug('infobot root', JSON.stringify(rt).slice(0, 500));
	...
	check_message(CHAN, text, can_lock, nick){
		...
		if(this.debug) CHAN.log.debug('lock', lock_regex, lock_factoid);
		...
		if(this.debug) CHAN.log.debug('forget', forget_regex, forget_factoid);
		...
		if(this.debug) CHAN.log.debug('get 1', get_regex, get_factoid);
		if(get_factoid !== null)
		{
		...
		if(this.debug) CHAN.log.debug('get 2', get_regex, get_factoid);
		if(get_factoid !== null)
		{
			var about = get_factoid[4].trim().replace(/\s/gi, '_');
			var determiner = get_factoid[3] ? get_factoid[3].trim() : null;
		...
		get_factoid = text.match(get_regex);
		if(this.debug) CHAN.log.debug('get 3', get_regex, get_factoid);
		if(get_factoid !== null)
		{
		...
		if(this.debug) CHAN.log.debug('append 1', append_regex, append_factoid);
		...
		if(this.debug) CHAN.log.debug('append 2', append_regex, append_factoid);
		if(append_factoid !== null)
		{
		...
		if(this.debug) CHAN.log.debug('set 1', set_regex, set_factoid);
		if(set_factoid !== null)
		{
		...
		if(this.debug) CHAN.log.debug('set 2', set_regex, set_factoid);
		if(set_factoid !== null)
		{
			var about = nick;
		...
		set_factoid = text.match(set_regex);
		if(this.debug) CHAN.log.debug('set 3', set_regex, set_factoid);
		if(set_factoid !== null)
		{
			var about = set_factoid[2].trim().replace(/\s/gi, '_');
			var determiner = set_factoid[1] ? set_factoid[1].trim() : null;
		...
		if(this.debug) CHAN.log.debug('question', question_regex, question_factoid);
