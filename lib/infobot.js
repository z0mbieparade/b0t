...
INFO.prototype.check_message = function(text, can_lock){
	...
	//what's X / who's X
	var get_factoid = text.match(/^(what|who)'*s\s([\w\d\s]+)/i);
	if(get_factoid !== null)
	{
	...
	//no, X is/are Y
	var update_factoid = text.match(/^(?:no,)\s([\w\d]+)\s(is|are)\s([^\n\r.!?]+)/i);
	...
	//X is/are also Z
	var append_factoid = text.match(/^([\w\d]+)\s(is|are)\s(?:also)\s([^\n\r.!?]+)/i);
	...
	//X is/are Y
	var set_factoid = text.match(/^([\w\d]+)\s(is|are)\s([^\n\r.!?]+)/i);
	if(set_factoid !== null)
	{
