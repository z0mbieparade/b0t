...
module.exports = class CMDS{
    ...
    verify_command(USER, cmd, options, callback){
        ...
        if(config.require_nickserv_to_edit_user_data && cmd_data.registered && USER.registered === false){
            b.channels[USER.chan].say({err: config.command_prefix + cmd + ' command requires registration with NickServ. If you just registered, try ' + config.command_prefix + 'list to renew user registration statuses.'});
