...
module.exports = class CMDS{
    ...
    verify_command(USER, cmd, options, callback){
        ...
        if(options.help){ //show long syntax
            ...
            if(config.require_nickserv_to_edit_user_data && cmd_data.registered && USER.registered === false) return_true += USER.t.fail('(ns)');
        }

        if(config.require_nickserv_to_edit_user_data && cmd_data.registered && USER.registered === false && options.show_nickserv_err){
            b.channels[USER.chan].say({err: config.command_prefix + cmd + ' command requires registration/ID with NickServ. If you just registered or ID\'d, try ' + config.command_prefix + 'list to renew user registration statuses.'});
