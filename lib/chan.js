                ...
                if(time_left > 0){
                    ...
                    if(time_left > 60000){
                        x.add_pong('polls_reminder' + _this.chan, 540000, function(cb_func){
                            ...
                            db.get_data('/polls/' + chan + '[-1]', function(poll){
                                if(poll !== null && poll.status === 'open'){
        ...
        if(this.config.speak_on_channel_join){
            ...
            if(enter_msg.length > 1 && enter_msg[0].toLowerCase() === 'qotd'){
                db.get_data('/topic', function(data){
                   if(data !== null){
                        var msg = x.rand_color(x.rand_arr(data), _this.config.discord_relay_channel || _this.config.disable_colors);
    ...
    message(nick, text, is_discord_user){
        ...
        if (text.match(new RegExp('^' + bot.nick)) !== null && this.config.respond_to_bot_name) { 
            ...
            if(command_args_org[0] == '-version') {
                ...
                say_my_name = 'uptime: ' + x.ms_to_time(uptime_ms, true, true);
            } else {
        ...
        //respond to command
            ...
            var command_data = b.cmds.command(command, (is_discord_user ? _this.discord_users[nick] : _this.users[nick]));
            ...
            b.cmds.verify_command((is_discord_user ? _this.discord_users[nick] : _this.users[nick]), command, {help: true, is_pm: false}, function(cmd){
                if(cmd !== false){

                    var command_args = b.cmds.parse_command_input((is_discord_user ? _this.discord_users[nick] : _this.users[nick]), _this, command, command_str);
