...
module.exports = class CHAN{
    ...
    message(nick, text, is_discord_user){
                ...
                if(is_yt !== null){
                     x.get_url(is_yt[5], 'youtube', function(data){
                        var str =  _this.t.highlight(data.title) + _this.t.null(' | Uploader: ') + _this.t.highlight(data.owner);
                            str += _this.t.null(' | Time: ') + _this.t.highlight(x.ms_to_time(data.duration * 1000, {short: false})) + _this.t.null(' | Views: ') + _this.t.highlight(data.views);  
        ...
        //respond to command
            ...
            var USER = (is_discord_user ? _this.discord_users[nick] : _this.users[nick]);
        ...
        if(options.col === null){
            ...
            Object.keys(_this.users).forEach(function(nick) {
                options.col.forEach(function(c){
                    ...
                    b.users.get_user_data(_this.users[nick], new_options, function(user_data){
                        if(user_data !== false){
                            rows[nick][c] = user_data;
                        }
                    });
                })
