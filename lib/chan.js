var merge       = require('merge'),
    User        = require(__plugindir + '/lib/user.js'),
    DUser       = require(__plugindir + '/lib/discord_user.js');
...
function CHAN(chan) {
    ...
    log4js.addAppender(log4js.appenders.file(__plugindir + '/logs/' + chan + '_' + b.log_date + '.log'), chan);
	...
	var config_custom = {}; 
	try {
	   config_custom = require(__plugindir + '/chan_config/./config_' + chan + '.json');
...
CHAN.prototype.init_chan = function(){
    ...
    if(_this.config.info_bot){
        var INFO    = require(__plugindir + '/lib/infobot.js').INFO;
    ...
    if(links !== null && _this.config.parse_links){
        for(var i = 0; i < links.length; i++) {
            ...
            if(is_yt && is_yt.length && is_yt[1]){
                 _this.action.get_url(is_yt[1], 'youtube', function(data){
                    var str =  _this.t.highlight(data.title) + _this.t.null(' | Uploader: ') + _this.t.highlight(data.owner);
                        str += _this.t.null(' | Time: ') + _this.t.highlight(_this.action.ms_to_time(data.duration * 1000, false)) + _this.t.null(' | Views: ') + _this.t.highlight(data.views);  
                    _this.SAY.say(str, 1, {to: _this.chan, ignore_bot_speak: true});
                });

            } else if(links[i].indexOf('imgur.com') > -1) {

                 _this.action.get_url(links[i], 'html', function(data){
                    var str_arr = [];
                ...
                if(!_this.config.discord_relay_channel){
                    _this.action.get_url(links[i], 'sup', function(data){
