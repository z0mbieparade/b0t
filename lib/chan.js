...
if(config.API.twitter &&
  ...
  config.API.twitter.api_secret !== '' &&
	config.API.twitter.bearer_token !== '') {

	var Twitter = require('twitter');
	var twitterClient = new Twitter({
		consumer_key: config.API.twitter.api_key,
		consumer_secret: config.API.twitter.api_secret,
		bearer_token: config.API.twitter.bearer_token
	});
} else {
	b.log.warn('Missing Twitter API key/secret/token! Smart links for Twitter will be disabled.');
	...
	message(nick, text){
		...
		if(links !== null && _this.config.parse_links){
			for(var i = 0; i < links.length; i++) {
				...
				if(is_yt !== null){
				...
				} else if(twitterClient && links[i].indexOf('twitter.com') > -1) {
					...
					twitterClient.get('https://api.twitter.com/2/tweets/' + twpath, function(error, tweets, response) {
						if (error) {
							b.log.error(error);
							return;
						}
						b.log.debug(tweets)
						let str = _this.t.null(tweets.data.text);
						_this.say(str, 1, { to: _this.chan, ignore_bot_speak: true });
					});
