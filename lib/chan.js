		...
		if(links !== null && _this.config.parse_links){
			for(var i = 0; i < links.length; i++) {
				x.get_url(links[i], 'sup', function(data){
					_this.say(_this.t.null(data), 1, {to: _this.chan, ignore_bot_speak: true, skip_verify: true});
				},{
					...
					if(cmd !== false){
						...
						if(command_args.err){
							...
							try {
								...
								_this.log.error('command_data.func (CHAN)', e, e.message);
	...
	get_all_users_in_chan_data(options, callback) {
		...
		if(options.col === null){
			...
			Object.keys(_this.users).forEach(function(nick) {
				...
				if(rows[nick] && Object.keys(rows[nick]).length > 0){
					count++;
				} else {
					delete rows[nick];
				}
