...
CHAN.prototype.init_chan = function(){
    ...
    db.get_db_data('/polls/' + _this.chan + '[-1]', function(poll){
        if(poll !== null && poll.status === 'open'){
            var now = (new dateWithOffset(0)).getTime();
            var time_left = (poll.time + 600000) - now;

            _this.log.debug('time left', x.ms_to_time(time_left, true, true));
            ...
            if(time_left > 0){
                x.add_pong('polls' + _this.chan, time_left, function(cb_func){
                    ...
                    x.close_current_poll(_this, function(result){
                        _this.SAY.say(result, 1, {to: _this.chan, skip_verify: true, ignore_bot_speak: true, skip_buffer: true, join: '\n'});
                    });
                }, true);
            } else {
                x.close_current_poll(_this, function(result){
...
CHAN.prototype.enter_msg = function(){
    var _this = this;
...
CHAN.prototype.nick_change = function(old_nick, new_nick){
    var _this = this;
    if(b.users[old_nick]){
        b.users[old_nick].nick_change(old_nick, new_nick);
    } else {
        _this.add_or_update_user(new_nick);
    }
}
...
CHAN.prototype.add_or_update_user = function(nick, perm, send_tell_msgs){
    ...
    x.whois(nick, function(whois, whois_short){

        if(!_this.users[whois.nick]){
            _this.users[whois.nick] = {
                nick: whois.nick,
                t: _this.t,
                CHAN: _this,
                perm: perm
            ...
            _this.log.info('>Locked and loaded', whois.nick)
        };

        if(send_tell_msgs) x.send_tell_messages(whois.nick);

        if(b.is_op && _this.users[whois.nick].perm === '' && _this.config.voice_users_on_join){
            bot.send('samode', _this.chan, '+v', whois.nick);
            _this.users[whois.nick].perm = '+';
        } else {
            _this.log.trace(bot.nick, 'is not opper, cannot samode +v');
        }

        b.users[whois.nick].test_chan_owner(_this.chan);
    });
}
...
CHAN.prototype.message = function(nick, text, is_discord_user){
    ...
    if(!is_discord_user){
        ...
        if(!_this.discord_users[nick]){
               _this.discord_users[nick] = {
                    nick: nick,
                    t: _this.t,
                    CHAN: _this,
                    perm: '',
            ...
            _this.log.info('> Synchronizing', nick, 'in', _this.chan, '(discord)'); 
        } 
    }
    ...
    //respond to command
        ...
        x.verify_command((is_discord_user ? _this.discord_users[nick] : _this.users[nick]), command, {help: true, is_pm: false}, function(cmd){
            if(cmd !== false){

                var command_args = x.parse_command_input((is_discord_user ? _this.discord_users[nick] : _this.users[nick]), _this, command, command_str);

                if(command_args.err){
                    _this.log.error(command_args.err);
                    _this.SAY.say(command_args.err === 'help' ? cmd : cmd + ' ' + _this.t.errors( '(' + command_args.err.join(', ') + ')'), 2, {skip_verify: true});
                } else {
...
CHAN.prototype.action = function(nick, text, is_discord_user){
    ...
    if(!is_discord_user){
        if(!_this.users[nick]) _this.add_or_update_user(nick, '', true);
    } else {
