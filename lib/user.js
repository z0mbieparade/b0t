    ...
    x.get_user_whois(_this.whois_short, false, b.channels[chan].config.chan_owner, function(result){
        if(result){
            ...
            if(b.is_op){
                ...
                b.channels[chan].log.warn(bot.nick, 'is not opper, cannot samode +q');
            } 
        }
    });
}
...
USER.prototype.say_tagline = function(chan){
    ...
    x.get_user_data(_this.nick, {
        ...
        if(b.channels[chan].config.discord_relay_channel){
            b.channels[chan].SAY.say(x.no_highlight(_this.nick) + ' has joined', 1, {skip_verify: true, ignore_bot_speak: true, skip_buffer: true});
        }
        if(tags !== false && tags.length && tags.length > 0){
            b.channels[chan].SAY.say(tags[x.rand_number_between(0, tags.length - 1)], 1, {skip_verify: true, ignore_bot_speak: true, skip_buffer: true});
        }
    });
}
