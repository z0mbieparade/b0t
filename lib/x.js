...
module.exports = class X{
    ...
    verify_command(USER, cmd, options, callback){
        ...
        var chan_config = USER.chan ? b.channels[USER.chan].config : config.chan_default;
        ...
        if(options.help){ //show long syntax
            ...
            if(command.perm && command.perm !== '') return_true += _this.t.success('(' + (command.perm === 'owner' ? 'α' : command.perm) + ')');
        }
        ...
        if(USER.is_discord_user || !USER.is_owner){
            ...
            if(command.spammy && options.is_pm !== true && options.ignore_spammy === false){
                _this.check_speak_timeout(USER.chan + '/spammy/' + cmd, b.channels[USER.chan].config.limit_spammy_commands, function(wait_ms){
                    if(wait_ms){
                        b.log.warn(USER.CHAN.chan, 'spammy, wait', _this.ms_to_time(wait_ms));
                        b.channels[USER.chan].SAY.say({err: 'spammy, wait ' + _this.ms_to_time(wait_ms, false) + ' before you can use ' + config.command_prefix + cmd + ' in chat.'});
                        callback(false);
                        return;
                    } else {
                        _this.update_speak_time(USER.chan + '/spammy/' + cmd, 1);
