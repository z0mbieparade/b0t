...
module.exports = class X{
    ...
    //smart tries to do a thing to shorten dates depending on how long it's been
    ms_to_time(duration, ms, short, smart) {
        ms = ms === false ? false : true;
        var Nmilliseconds    = parseInt((duration%1000)/100), 
            Nseconds         = parseInt((duration/1000)%60), 
            Nminutes         = parseInt((duration/(1000*60))%60), 
            Nhours           = parseInt((duration/(1000*60*60))%24),
            Ndays            = duration >= (1000*60*60*24) ? parseInt((duration/(1000*60*60*24))%7) : 0,
            Nweeks           = duration >= (1000*60*60*24*7)  ? parseInt((duration/(1000*60*60*24*7))%4) : 0,
            Nmonths          = duration >= (1000*60*60*24*7*4) ? parseInt((duration/(1000*60*60*24*7*4))%12) : 0,
            Nyears           = duration >= (1000*60*60*24*7*4*12) ? parseInt((duration/(1000*60*60*24*7*4*12))) : 0;

        //1 day
        if(duration >= 86400000 && !short) b.log.warn(duration, 'greater than 1 day, not handled use "short:true"');

        if(short){
            var years           = Nyears > 0 ? Nyears + 'yr' : '',
                months          = Nmonths > 0 ? Nmonths + 'mo' : '',
                weeks           = Nweeks > 0 ? Nweeks + 'wk' : '',
                days            = Ndays > 0 ? Ndays + 'd' : '',
                hours           = Nhours > 0 ? Nhours + 'h' : '', 
                minutes         = Nminutes > 0 ? Nminutes + 'm' : '', 
                seconds         = Nseconds > 0 ? Nseconds + 's' : '',
                milliseconds    = ms && Nmilliseconds > 0 ? Nmilliseconds + 'ms' : '';
            ...
            hours           = (Nhours < 10) ? "0" + Nhours : Nhours;
            minutes         = (Nminutes < 10) ? "0" + Nminutes : Nminutes;
            seconds         = (Nseconds < 10) ? "0" + Nseconds : Nseconds;

            return hours + ":" + minutes + ":" + seconds + (ms ? "." + Nmilliseconds : '');
        }
    }
