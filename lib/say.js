...
module.exports = class SAY{
	...
	table(data, options){
		...
		options = Object.assign({}, {
			title: null,
			header: true, //if true, we add a header row with the col titles. can also be an object {key: 'title'}
		...
		if(options.header){
			...
			{
				let col_txt = col;
				if(typeof options.header === 'object' && options.header[col]) col_txt = options.header[col];

				var scs_col = scs(col_txt, true);
				if(col !== 'cluster_id' && col.match(/.*?_hidden$/i) === null)
				{
					avr_widths[col] = avr_widths[col] ? avr_widths[col] + scs_col.len : scs_col.len;
				}

				if(((widths[col] !== undefined && widths[col] < col_len) || widths[col] === undefined) &&
					(col !== 'cluster_id' && col.match(/.*?_hidden$/i) === null))
				{
					widths[col] = scs_col.len;
				}
			}
		}
		...
		function resize_col(row, col, text, length, join, unicode_replace){
			unicode_replace = unicode_replace === false ? false : true;
			var scs_txt = scs(text, unicode_replace);
		...
		function create_row(row, unicode_replace){
			unicode_replace = unicode_replace === false ? false : true;
			var div = scs_div.txt;
			var pre = '';
			...
			if(Array.isArray(row)){
				for(var i in row){
					let len = widths[row[i]];
					if(!len && typeof options.header === 'object')
					{
						for(var k in options.header)
						{
							if(options.header[k] === row[i])
							{
								len = widths[k];
								break;
							}
					...
					row_arr.push(resize_col(null, null, row[i], len, ' ', unicode_replace));
				}
			} else {
				for(var col in widths){
					...
					var resize = resize_col(row, col, cell_data, widths[col], ' ', unicode_replace)
					row_arr.push(resize);
				}
			}
		...
		if(options.header){
			if(typeof options.header === 'object'){
				let header_cols = Object.keys(widths).map(function(w){
					return options.header[w] ? options.header[w] : w;
				});
				new_data.push(create_row(header_cols, false));
			} else {
