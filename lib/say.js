    ...
    function check_command_count(callback){
        db.get_db_data("/speak/" + _this.chan + '/cmd/' + options.nick, function(cmd_arr){
            if(cmd_arr !== null && cmd_arr.length > 0){
                var epoc = (new dateWithOffset()).getTime();
...
SAY.prototype.table = function(data, options){
    ...
    var cluster_col = [c.red, c.olive, c.yellow, c.green, c.teal, c.blue, c.purple, c.pink];
    var widths = {};
    var new_data = [];

    function scsl(txt){
        var new_txt = txt === undefined ? '' : typeof txt === 'string' ? c.stripColorsAndStyle(txt) : txt + '';
        return [...new_txt].length; //this seems to get rid of some weird issues with string length
    }
    ...
    function create_row(row){
        ...
        if(Array.isArray(row)){
            ...
            for(var col in widths){
                row_arr.push(resize_col(row[col], widths[col]));
            }
        }
    ...
    for(var row in data){
        if(data[row].cluster_id && data[row].cluster_id !== cluster_id){
            ...
            cluster_col.push(cluster_col.shift());
        }
