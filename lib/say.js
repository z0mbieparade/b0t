...
SAY.prototype.say = function(){
    ...
    function check_chan_busy_status(callback){
        x.owner_nick(false, function(owner_nick){
            if(options.ignore_bot_speak === false &&
               ...
               (options.to !== owner_nick || owner_nick === null) && 
               options.is_pm === false){ 
    ...
    function update_chan_speak_status(){
        x.owner_nick(false, function(owner_nick){
            if(options.ignore_bot_speak === false &&
               options.is_cmd === true && 
               (options.nick !== owner_nick || owner_nick === null) && 
               options.is_pm === false){
...
SAY.prototype.table = function(data, options){
    ...
    function scs(txt, unicode_replace){
        ...
        var new_txt = c.stripColorsAndStyle(txt);
            ...
            if(unicode_replace) new_txt = new_txt.replace(/(?![\s\u{2600}-\u{26FF}\u{1F300}-\u{1F5FF}])[\u{0100}-\u{FFFF}]/gu, '?');

        var new_txt_len = [...new_txt].length; //this seems to get rid of some weird issues with string length
        return {txt: new_txt, len: new_txt_len};
    }
    ...
    for(var row in data){
        ...
        for(var col in data[row]){
            var scs_col = scs(data[row][col], true);
            if(col !== 'cluster_id' && col.match(/.*?_hidden$/i) === null) avr_widths[col] = avr_widths[col] ? avr_widths[col] + scs_col.len : scs_col.len;
