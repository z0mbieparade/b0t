...
USE.prototype.parse_command_input = function(USER, CHAN, cmd, text){
    ...
    function map_input(ps, input, or){
        ...
        for(var p = 0; p < ps.length; p++){
            ...
            if(param.type && param.type !== ''){
                ...
                if(matched !== null){
                    ...
                    if(def !== undefined && def !== null && def !== ''){
                        ...
                        if(opt === false && or !== true){ //not optional, not OR
                            param.error = ps[p].error || [];
                            param.error.push('REQUIRED');
                            errors.push('Missing ' + param.name);
                        } else {
                            delete_param('no match, optional:' + opt + ' or:' + or);
                            continue;
                        } 
            ...
            if(param.or){
                var orr = map_input(param.or, input, true); 
                param.or = orr;
            ...
            if(param.and) param.and = map_input(param.and, and_input, false);
        }
...
USE.prototype.verify_command = function(USER, cmd, options, callback){
    ...
    var return_true = options.help ? _this.cmd_syntax(USER, cmd, options) : 
                    (options.is_pm && command.no_pm ? _this.t.null(config.command_prefix + cmd) : config.command_prefix + cmd);
...
USE.prototype.manage_arr = function(USER, path, args, cmd, callback){
    var _this = this;
...
USE.prototype.search_arr = function(USER, path, args, rand, callback){
    var _this = this;
