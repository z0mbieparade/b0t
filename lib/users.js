		...
		if(b.is_op && config.autokb_users_inactive_for > 0){
			...
			nicks.forEach(function(nick){
				...
				_this.get_user_data(nick, {
                    ...
                    if(!data.seen){
                        b.log.warn('autokb_check_list', nick, 'has never been seen');
                    } else {
                        if(now - config.autokb_users_inactive_for > data.seen.date)
                        {
                        	var absent_for = now - data.seen.date;
                        	var kb_inactive_for = x.ms_to_time(config.autokb_users_inactive_for, false, true, true);

                        	b.log.warn('autokb_check_list', nick, 'has been absent over', kb_inactive_for, 'absent for', x.ms_to_time(absent_for, false, true), absent_for);

                        	bot.send('mode', chan, '+b', nick);
                        	bot.send('kick', chan, nick, 'You have been idle for over ' + kb_inactive_for + ', to rejoin ' + chan + ' please type: /msg ' + bot.nick + ' ' + config.command_prefix + 'unban' );
