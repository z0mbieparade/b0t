	...
	who_reply(msg){
		...
		switch(msg.rawCommand){
	        ...
	        case 'NOTICE': //for nickserv register check
	        	...
	        	var ns_msg = c.stripColorsAndStyle(msg.args[1]);

	        	if(ns_msg.match(/isn't registered/g)){
	        		user_data = {
	        			nick: ns_msg.replace(/Nick | isn't registered./ig, ''),
	        			registered: false
	        		}
	        		user_data.who = user_data.nick + '!*@*';
	        	} else if(ns_msg.match(/(.*?) is (.*)/g) && !ns_msg.match(/is currently online/ig)){
	        		var dat = ns_msg.split(' is ');
