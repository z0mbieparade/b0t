...
module.exports = class USERS{
	...
	new_info_notice(notice){
		...
		var times = Object.keys(this.notice_queue);
		times.forEach(function(t){
			if(t >= time - 5 && t <= time) time = t;
		});
	...
	parse_notice(time, msg_arr){
		...
		b.log.debug('parse_notice', user_data);

		if(user_data.nick){

			if(this.nicks_awaiting_info_queue[user_data.nick]){
				delete this.nicks_awaiting_info_queue[user_data.nick]
			} else {
				for(var nick in this.nicks_awaiting_info_queue){
					if(nick.match(new RegExp('^' + user_data.nick + '$', 'i'))){
						delete this.nicks_awaiting_info_queue[nick]
						user_data.nick = nick;
					}
				}
			}
	...
	autokb_check_list(nicks, chan){
		var _this = this;
		var chan_config = b.channels[chan].config;

		if(b.is_op && chan_config.autokb_users_inactive_for > 0){
			var now = (new dateWithOffset(0)).getTime();
			...
			nicks.forEach(function(nick){
				...
				_this.get_user_data(nick, {
                	...
                	var last_seen = data.seen && data.seen.date ? data.seen.date : false;
					...
					if(!last_seen && !last_spoke){
                    	...
                    	var kb_inactive_for = x.ms_to_time(chan_config.autokb_new_user_threshold, false, true, true);
                        b.log.warn('autokb', nick, 'has never said anything: autokb_new_user_threshold', chan_config.autokb_new_user_threshold, kb_inactive_for);
                        ...
                        {
                        	var absent_for = now - last_seen;

                        	b.log.warn('autokb (new user)', nick, 'has not spoken', kb_inactive_for, 'absent for', x.ms_to_time(absent_for, false, true), absent_for);

                        	bot.send('mode', chan, '+b', nick);
                        	bot.send('kick', chan, nick, 'You have not spoken for over ' + kb_inactive_for + ', to rejoin ' + chan + ' please type: /msg ' + bot.nick + ' ' + chan_config.command_prefix + 'unban' );
                    	...
                    	var kb_inactive_for = x.ms_to_time(chan_config.autokb_users_inactive_for, false, true, true);
                    	b.log.warn('autokb', nick, 'has not spoken in over: autokb_users_inactive_for', chan_config.autokb_users_inactive_for, kb_inactive_for);

                        if(now - chan_config.autokb_users_inactive_for > last_spoke)
                        {
                        	var absent_for = now - last_spoke;
                        	b.log.warn('autokb', nick, 'has not spoken for over', kb_inactive_for, 'absent for', x.ms_to_time(absent_for, false, true), absent_for);
	...
	get_user_data(USER, options, callback) {
	    ...
	    function get_usr_data(path){
		    ...
		    }, (options.use_nick_org ? nick_org : nick));
		}
	}
