	...
	who_reply(msg){
		...
		switch(msg.rawCommand){
			...
			case '352': //who
				...
				if(msg.args[4].match(/^(\d+\.){3}(\d+)$/)){
					user_data.ip = msg.args[4];
				} else {
					user_data.ip_mask = msg.args[4];
				}

				break;
			case '302': //userhost
				var split = msg.args[1].split(/[*=+@]+/).map(part => part.trim());
				user_data = {
					who: split[0] + '!' + split[1] + '@*',
					nick: split[0],
					user: split[1],
				...
				if(split[2].match(/^(\d+\.){3}(\d+)$/)){
					user_data.ip = split[2];
				} else {
					user_data.ip_mask = split[2];
				}
					...
					if(usr.where === 'on_server'){
						...
						if(user_data.who){
							...
							if(!_this.who_queue[user_data.nick]){
								...
								bot.send('USERHOST', user_data.nick);
				...
				_this.get_user_data(nick, {
					...
					if(!last_seen && !last_spoke){
						...
						if(now - chan_config.autokb_new_user_threshold > last_seen)
						{
							...
							var kb_inactive_for = x.ms_to_time(chan_config.autokb_new_user_threshold);

							b.log.warn('autokb (new user)', nick, 'has not spoken', kb_inactive_for, 'absent for', x.ms_to_time(absent_for), absent_for);
						...
						if(now - chan_config.autokb_users_inactive_for > last_spoke && now - chan_config.autokb_new_user_threshold > last_seen)
						{
							var absent_for = now - last_spoke;
							var kb_inactive_for = x.ms_to_time(chan_config.autokb_users_inactive_for);

							b.log.warn('autokb', nick, 'has not spoken for over', kb_inactive_for, 'absent for', x.ms_to_time(absent_for), absent_for);
