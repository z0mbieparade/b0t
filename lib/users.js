...
module.exports = class USERS{
	...
	get_user_data(USER, options, callback) {
	    ...
	    if(!cmd_data.err){
	        ...
	        if(options.register_syntax === null && !options.skip_say && !cmd_data.err) options.register_syntax = b.cmds.cmd_syntax({}, options.col, {short: true});
	    }
	    ...
	    this.get_user_data(nick, {
	        ...
	        if(messages !== false){
	            for(var sender in messages){
	                b.pm.say(sender + ' said to tell ' + nick + ' ' + messages[sender].join(' | '), 3, {to: nick});
	                b.pm.say({succ: 'told ' + nick + ' ' + messages[sender].join(' | ')}, 3, {to: sender});
	            }
