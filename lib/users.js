...
module.exports = class USERS{
	...
	nickserv_check_list(nicks, chan, cb_name){
		...
		//console.log('1 nickserv_check_list', nicks, chan, cb_name);
		...
		//console.log('2 nicks_check_info_queue', _this.nicks_check_info_queue, 'nicks_awaiting_info_queue', _this.nicks_awaiting_info_queue);
        ...
        if(_this.nicks_check_info_queue.length > 0)
        {
        	...
        	if(_this.nicks_awaiting_info_queue.indexOf(nick) < 0){
	            _this.nicks_awaiting_info_queue.push(nick);
	            bot.say(config.nickserv_nick, 'info ' + nick);
	        }
        }

        //console.log('4 nicks_check_info_queue', _this.nicks_check_info_queue, 'nicks_awaiting_info_queue', _this.nicks_awaiting_info_queue);

        if (_this.nicks_check_info_queue.length > 0 || _this.nicks_awaiting_info_queue.length > 0 || times.length > 0){
	...
	parse_notice(time, msg_arr){
		...
		if(user_data.nick){
			...
			if(this.nicks_awaiting_info_queue.indexOf(user_data.nick) > -1)
			{
				this.nicks_awaiting_info_queue.splice(this.nicks_awaiting_info_queue.indexOf(user_data.nick), 1)
			}
		}
	}
