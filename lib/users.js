...
module.exports = class USERS{
	...
	autokb_check_list(nicks, chan){
		var _this = this;
		if(b.is_op && config.autokb_users_inactive_for > 0){
			var now = (new dateWithOffset(0)).getTime();
			...
			nicks.forEach(function(nick){
				...
				_this.get_user_data(nick, {
					...
					if(!last_seen && !last_spoke){
                    	...
                    	var kb_inactive_for = x.ms_to_time(config.autokb_new_user_threshold, false, true, true);
                        b.log.warn('autokb', nick, 'has never said anything: autokb_new_user_threshold', config.autokb_new_user_threshold, kb_inactive_for);
                        //this is where our threshold comes into effect, as technically this should only apply to new users

                        if(now - config.autokb_new_user_threshold > last_seen)
                        {
                    	...
                    	var kb_inactive_for = x.ms_to_time(config.autokb_users_inactive_for, false, true, true);
                    	b.log.warn('autokb', nick, 'has not spoken in over: autokb_users_inactive_for', config.autokb_users_inactive_for, kb_inactive_for);

                        if(now - config.autokb_users_inactive_for > last_spoke)
                        {
