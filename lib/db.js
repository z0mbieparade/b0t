...
module.exports = class DB{
    constructor(options){
        ...
        options = Object.assign({}, {
            ...
            on_empty: function(){
                try {
                    ...
                    return require(__botdir + '/config/./default_' + options.db_name + '.json');
                } catch (e) {
        ...
        this.jdb = new JsonDB(__botdir + '/' + options.db_name + '.json', true, options.readable);
        ...
        this.get_data('/', function(db_root){
            ...
            //b.log.debug(options.db_name, db_root);
            ...
            if(db_root === null || 
                ...
                db_root = options.on_empty();
                //b.log.debug('empty', options.db_name, db_root);
            }

            db_root = options.on_load(db_root);

            _this.update('/', filter_empty(db_root), true, function(){
                b.log.info('Engaged the quantum ' + options.db_name + ' database');
            });
        });
    }
    ...
    manage_arr(USER, path, args, callback, case_insensitive){
        var _this = this;
        ...
        _this.get_data(path, function(data){
            if(data && data.length > 0){
                if(args.id !== undefined){
                    ...
                    if(rand){
                        callback(data[x.rand_number_between(0, data.length - 1)], 1);
                    } else {
