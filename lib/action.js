...
ACT.prototype.say = function(msg, level, options){
    options = Object.assign({}, {
        ...
        ignore_discord_formatting: false //if true, doesn't strip all unicode from string in discord chan
    }, options);
    ...
    if(action.is_discord_user === true && options.to !== config.owner){
        level = 1;
        options.to = action.chan;
        options.lines = options.force_lines === false ? 4 : options.lines;
    } else {
        if(options.to === null){
    ...
    var say_str = function(str, opt){
        ...
        if((config.discord_relay_channels && config.discord_relay_channels.indexOf(options.to) > -1 && options.ignore_discord_formatting === false) || 
        ...
        this.update_db('/buffer/' + options.to, new_data_obj, true, function(act){
            if(act === 'add'){
                action.page_buffer(options, function(data, new_opt){
                   ...
                   if(action.is_pm()){
                        data = t.highlight('To page through buffer, type ' + config.command_prefix + 'next. (type ' + config.command_prefix + 'next help for more info)\n') + data;
                    }
...
ACT.prototype.page_buffer = function(options, callback){
    ...
    if(options.join === '\n'){
        ...
        options.join = ' ' + t.highlight(options.join) + ' ';
        options.lines = options.lines !== undefined && +options.lines < 21 && +options.lines > 0 ? options.lines : 5; 
    }
    ...
    this.get_db_data('/buffer/' + options.to, function(old_buffer){
        if(old_buffer !== null && old_buffer !== undefined && old_buffer !== '' && old_buffer.length > 1){
            ...
            options.next_info = t.highlight('(' + (old_buffer[0].first_len - (old_buffer.length - 1)) + '/' + old_buffer[0].first_len + ')');
...
ACT.prototype.cmd_syntax = function(cmd, short, micro){
    ...
    if(micro){
        ...
        if (optional) syntax += t.warn(' [optional]');
        if (cm.colors) syntax += c.rainbow(' colors');
        return syntax;
    } else {
        if (optional) syntax += t.warn(' [params are optional]');
        if (cm.colors) syntax += c.purple(' (accepts ') + c.rainbow('colors!') + c.purple(')');

        return t.highlight('Usage: ') + syntax + ' ' + t.highlight('Description: ') + cm.action + '.';
    }
}
...
ACT.prototype.verify_command = function(cmd, help){
    ...
    if(command.discord !== undefined && command.discord === false && action.is_discord_user === true){
        log.debug('skipping ' + config.command_prefix + cmd + ' because disabled for discord users');
        return false;
    }
...
ACT.prototype.get_user_data = function(nick, options, callback) {
    ...
    try{
        ...
        if(!options.skip_say){
            var str = t.fail(nick + ', your ') + t.highlight(options.label) + t.fail(' is not registered, please type ') + t.highlight(options.register_syntax) + t.fail(' to register it');
...
ACT.prototype.get_all_users_in_chan_data = function(options, callback) {
    ...
    if(!names[options.chan]){
        action.say({'err': 'No users registered with ' + t.highlight(options.label) + ' currently in the channel'});
    }
...
ACT.prototype.update_user = function(nick, data, callback) {
    try{
        ...
        if(data.data === ''){
            callback({msg: t.highlight(nick) + '\'s ' + data.col + ' has now been removed', act: 'remove'});
        } else {
            callback({msg: t.highlight(nick) + '\'s ' + data.col + ' has now been set', act: 'add'});
        }
    } catch(e) {
        log.warn(e.message);
    }
...
ACT.prototype.random_number_between = function(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;;
};

//formats a string or array with a random color (excludes white, black, bold, italic, underline, reset)
ACT.prototype.rand_color = function(data) {
    if(data === undefined){
        log.error('data undefined could not color');
        return;
    }
    ...
    if(config.disable_colors){
        ...
        var c = '\u0003' + col_arr[action.random_number_between(0, col_arr.length - 1)];
...
ACT.prototype.manage_arr = function(path, args, new_data, cmd){
    var type = args[0] === '-list' || args[0] === '-edit' || args[0] === '-delete' ? args[0] : '-add';
    var id = null;
    new_data = type === '-edit' && new_data !== 'undefined' && new_data !== '' && new_data !== null ? new_data.replace(/^-edit\s\d+\s/, '') : new_data.trim();

    if((type === '-list' || type === '-edit' || type === '-delete') && action.is_discord_user){
        action.say({err: 'Discord users cannot use the ' + type + ' param.'});
        return;
    }

    if((type === '-edit' || type === '-delete') && (args.length < 2 || isNaN(args[1]) === true)){
        action.say({err: 'id is required to use ' + type + '!'}, 2);
        return;
    } else {
        id = args[1];
    }   
    ...
    if(type === '-edit' && (args.length < 3 || new_data === undefined || new_data === '' || new_data === null)){
        action.say({err: 'correct usage of command with edit is -edit <id> <new value>'}, 2);
        return;
    }

    if(type === '-add' && (new_data.length < 2 || new_data === '' || new_data === undefined || new_data === null)){
        action.say({err: t.highlight(action.cmd_syntax(cmd, true))}, 2);
        return;
    }

    var loop_thru = function(){
        action.get_db_data(path, function(d){

            if(d === null || d.length < 1){
                log.warn('nothing to loop thru in ' + path);
                action.say({err: 'No data to ' + type});
                return;
            }
            ...
            for(var i = 0; i < d.length; i++){
               ...
               if(id !== null && (i + 1) == id){
                    if(type === '-delete'){
                        action.delete_from_db(path + '[' + i + ']', function(deleted){
                            if(deleted){
                                matched = true;
                                action.say(str + t.success(' Deleted!'), 2);
                            } 
                        })
                    } else if(type === '-edit'){
                        d[i] = new_data;
                        action.update_db(path, d, true, function(act){
                            matched = true;
                            action.say('Updated ' + str + ' -> ' + new_data, 3);
                        });
                    }
                } else if (id == null) {
                    matched = true;
            ...
            if (matched === false) {
                action.say({err: 'No value by that id found'}, 3);
            } 
        }, true);
    }
    ...
    if(type !== '-add'){
        ...
        action.update_db(path, [new_data], false, function(act){
            action.say(t.highlight('Added!'), 3);
        });
    }
};
...
ACT.prototype.search_arr = function(path, args, search_for, rand, callback){
    var type = '-random';
    var id = null;
    ...
    if(args.length > 0 && isNaN(args[0]) === false){
        ...
        if(action.is_discord_user){
            action.say({err: 'Discord users cannot search.'});
            return;
        }
    }

    action.get_db_data(path, function(data){
            ...
            if(type === '-id'){
                ...
                for(var i = 0; i < data.length; i++){
                    if(data[i].toLowerCase().indexOf(search_for.toLowerCase().trim()) > -1){
                        ...
                        msg_found.push(t.warn('[' + i + '] ') + data[i]);
                    }
                }
                ...
                if(rand){
                    callback(data[action.random_number_between(0, data.length - 1)], 1);
                } else {
...
ACT.prototype.update_topic = function(chan){
    action.get_db_data('/topic', function(data){
        action.get_db_data('/pinned/'+chan, function(pinned){
            ...
            if(pinned){
                ...
                var pin_topic = '📌 ' + t.fail(c.stripColorsAndStyle(pinned[0]));
