...
ACT.prototype.say = function(msg, level, options){
    ...
    var say_str = function(str, opt){
        ...
        if(config.discord_relay_channels && config.discord_relay_channels.indexOf(options.to) > -1){
            log.debug('DISCORD CHAN', options.to);
    ...
    action.get_db_data('/topic', function(data){
        action.get_db_data('/pinned/'+chan, function(pinned){
            ...
            if(pinned){
                ...
                log.debug(topic, pinned, topic.indexOf(pinned[0]));
