...
ACT.prototype.say = function(msg, level, options){
    ...
    if(msg && msg.err){
        action.say(this.er(msg.err), 2, {skip_verify: true, skip_buffer: true});
        return;
    }
    ...
    if(options.to === null){
        ...
        options.lines = level === 1 ? 2 : options.lines;
    }
    ...
    var get_buffer = function(callback){
        if(options.page_buffer === true){
            action.page_buffer(options, function(data){
                callback(data);
            });
        } else {
            callback();
...
ACT.prototype.add_to_buffer = function(data, options, callback){
    ...
    if(new_data_obj.length <= 5){
        callback(new_data_obj.join('\n'));
    } else {
        ...
        this.update_db('/buffer/' + options.to, new_data_obj, true, function(act){
            if(act === 'add'){
                action.page_buffer(options, function(data){
                    ...
                    var append = '';
                    if(options.to === action.nick){
                        append = c.teal('To page through buffer, type ' + config.command_prefix + 'next. (type ' + config.command_prefix + 'next help for more info)\n');
                    }
...
ACT.prototype.page_buffer = function(options, callback){
    ...
    this.get_db_data('/buffer/' + options.to, function(old_buffer){
        if(old_buffer !== null && old_buffer !== undefined && old_buffer !== '' && old_buffer.length > 1){
            ...
            var next_info = c.teal('(' + (old_buffer[0].first_len - (old_buffer.length - 1)) + '/' + old_buffer[0].first_len + ')');

            var join = options.join === ' ' && old_buffer[0].join !== ' ' ? old_buffer[0].join : options.join;

            send_data.push((join === '\n' ? '' : '...\n') + (options.url === '' ? '' : options.url + ' ') + next_info);
