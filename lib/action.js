...
//data_obj should be an array. Array items over 200 chars are converted to another array item
ACT.prototype.add_to_buffer = function(data_obj, callback){
    var new_data_obj = [];
    if(data_obj === null || data_obj === false || data_obj === '' || data_obj.length < 1){
        new_data_obj = '';
    } else {
        data_obj.forEach(function(item, i){
        var item_arr = [];
        var split_string = function(str){
                if(str.length > 200){
                    var str_slice = str.slice(0, 200);
                    str = str.slice(201, str.length);
                    item_arr.push(str_slice);
                    split_string(str);
                } else {
                    item_arr.push(str);
                }
            ...
            split_string(item);
            new_data_obj = new_data_obj.concat(item_arr);
            
        });
        new_data_obj.unshift(new_data_obj.length);
    }

    this.update_db('/nicks/' + action.nick + '/buffer', new_data_obj, true, function(act){
        if(act === 'add'){
            action.say(c.teal('To page through buffer, type ' + config.command_prefix + 'next. (type ' + config.command_prefix + 'next help for more info)'), 3);
            log.debug('added to ' + action.nick + '\'s buffer!')
        } else {
            log.debug('cleared ' + action.nick + '\'s buffer!')
        }
    });
...
//lines = lines to send, default 5. Max 10, unless join is not a new line. Then max is 20.
//join = what to join lines by, default is new line.
ACT.prototype.page_buffer = function(lines, join){
    join = join || '\n';
    if(join !== '\n') {
        join = ' ' + c.teal(join) + ' ';
        lines = lines !== undefined && +lines < 21 && +lines > 0 ? lines : 5; 
    } else {
        lines = lines !== undefined && +lines < 11 && +lines > 0 ? lines : 5; 
    }

    this.get_db_data('/nicks/' + action.nick + '/buffer', function(buffer){
        if(buffer !== null && buffer !== undefined && buffer !== '' && buffer.length > 1)
        {
            var send_data = buffer.splice(1, lines);
            var next_info = c.teal('(' + (buffer[0] - (buffer.length - 1)) + '/' + buffer[0] + ')');

            send_data.push(next_info);

            action.say(send_data.join(join), 3, {skip_verify: true});


            buffer = buffer.length === 1 ? '' : buffer;
            action.update_db('/nicks/' + action.nick + '/buffer', buffer, true, function(act){
                if(act === 'add'){
                    log.debug('updated ' + action.nick + '\'s buffer!')
                } else {
                    log.debug('cleared ' + action.nick + '\'s buffer!')
                }
            });
        } else {
            action.say({'err': 'No buffer to page through.'}, 3);
        }
    });
}
...
//options.only_return_text : if true and type = html, instead of returning an array of dom object, returns an array of text only.
//only_return_nodes: {attr: {'class': 'zoom'}}  searched for only these values and returns them
                ...
                onopentag: function(name, attribs){
                    ...
                    if(options.only_return_nodes){
                        ...
                        if(options.only_return_nodes.tag){
                            for(var tag in options.only_return_nodes.tag){
                                if(name !== tag) return;
                            }
                        }
                    }
...
ACT.prototype.verify_command = function(category, cmd, help){
    ...
    if(commands[category].cmds[cmd].disabled || (cmd_override[cmd] && cmd_override[cmd] === "disabled")){
    ...
    if(commands[category].cmds[cmd].settings){
        for(var i = 0; i < commands[category].cmds[cmd].settings.length; i++){
            var setting_cat = commands[category].cmds[cmd].settings[i];
    ...
    if(action.nick !== config.owner){
        ...
        var test_perm = commands[category].cmds[cmd].perm || '';
        if(cmd_override[cmd]){
    ...
    if(help) {
        ...
        if(commands[category].cmds[cmd].params && commands[category].cmds[cmd].params.length > 0) {
...
ACT.prototype.get_user_data = function(nick, data, callback, skip_say) {
    var path = "/nicks/"+nick+'/'+data.col;
    try{
        var user_data = jdb.getData(path);
        callback(user_data);
    } catch(e) {
        if(!data.ignore_err) log.error('no user data found for', nick, 'in', path);
        if(!skip_say){
