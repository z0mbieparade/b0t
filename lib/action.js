...
ACT.prototype.get_url = function(url, type, callback, options){
    options = Object.assign({}, {
        ...
        only_return_nodes: null //{attr: {'class': 'zoom'}}  searched for only these values and returns them
    }, options);
...
//short default false, returns full syntax, true returns just !cmd <param>
ACT.prototype.cmd_syntax = function(cmd, short){
    short = short || false;
...
ACT.prototype.get_user_data = function(nick, options, callback) {
     ...
     if(command_by_plugin[options.col]){
        ...
        if(options.register_syntax === null && 
            !options.skip_say &&
            command_by_plugin[options.col] !== undefined) options.register_syntax = action.cmd_syntax(options.col, false);
     }
    ...
    try{
        ...
        if(!options.skip_say){
            var str = 'Your ' + c.teal(options.label) + ' is not registered! Please type ' + c.teal(options.register_syntax) + ' to register it';
